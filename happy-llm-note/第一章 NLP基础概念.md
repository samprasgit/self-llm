...
- **1950年**: 艾伦·图灵提出了图灵测试，他说如果一台机器可以通过使用打字机成为对话的一部分，并且能够完全模仿人类，没有明显的差异，那么机器可以被认为是能够思考的。这是**判断机器是否能够展现出与人类不可区分的智能行为**的测试。

- **诺姆·乔姆斯基** 提出了生成语法理论，认为**人类语言由一套内在的语法规则生成，而非单纯依赖经验模仿**，这对理解机器翻译的工作方式产生了重要影响。
...
子词切分（Subword Segmentation）是一种文本预处理技术，旨在**将词汇进一步分解为更小的单位**，即子词。特别适用于处理词汇稀疏问题，即当遇到罕见词或未见过的新词时，能够通过已知的子词单位来理解或生成这些词汇。
...
词性标注（Part-of-Speech Tagging，POS Tagging）的目标是为文本中的每个单词分配一个**词性标签**，如名词、动词、形容词等。
...
**主要类型**
  ...
  - 新闻分类: 体育、科技、娱乐、政治等
  - 情感细分: 非常积极、积极、中性、消极、非常消极
  - 意图识别: 查询、投诉、建议、咨询等

  3. 多标签分类

  - 电影标签: 一部电影可能同时属于"动作"、"冒险"、"科幻"
  - 文章标签: 一篇文章可能涉及多个主题
  ...
  **技术方法**
  ...
  - RNN/LSTM: 处理序列信息
  - CNN: 捕获局部特征
  - Transformer: BERT、RoBERTa、GPT等预训练模型
